...
+validates_exclusion_of+ 헬퍼는 +:in+ 옵션을 통해서 검증 대상 속성이 허락하지 않는 값들의 집합을 받습니다. +:in+ 옵션은 +:within+의 별명(alias)을 가지고 있는데, 이 이름이 사용하기 좋다면, 같은 목적으로 +:within+을 사용할 수 있습니다. 이 예제는 +:message+을 이용해서, 여러분이 속성의 값을 메세지 속에 포함시키는 방법을 보여줍니다.

+validates_exclusion_of+를 위한 기본 에러 메세지는  "_is reserved_" 입니다.

h4. +validates_format_of+ (포맷 검증)

이 헬퍼는 +:with+ 옵션으로 입력된 정규식을 이용해서 속성으 ㅣ값을 검증합니다.
...
+validates_inclusion_of+ 헬퍼는 +:in+ 옵션을 통해서 검증 대상 속성이 허락하는 값들의 집합을 받습니다. +:in+ 옵션은 +:within+의 별명(alias)을 가지고 있는데, 이 이름이 사용하기 좋다면, 같은 목적으로 +:within+을 사용할 수 있습니다. 이 예제는 +:message+을 이용해서, 여러분이 속성의 값을 메세지 속에 포함시키는 방법을 보여줍니다.
...
정규식으로 속성의 값을 검증할 것입니다. 다른 방법으로 +Float+를 이용해서 숫자 변환을 시도할 것입니다.
...
+:scope+ 옵션으로 유일성 검사 제한에 다른 속성을 지정해서 사용할 수 있습니다.:
...
모든 헬퍼가 사용할 수 있는 공통 옵션이 몇가지 있습니다. +:if+와 +:unless+를 제외한 이들 옵션은 나중에 "상태에 따른 검증":#conditional-validation 에서 논의 될 것입니다.
...
이미 보았던대로, +:message+ 옵션은 검증이 실패했을때 +errors+ 컬렉션에 추가되는 메세지를 지정합니다. 이 옵션을 사용하지 않으면, 엑티브 레코드는 각 검증 헬퍼들을 위한 각자의 기본 메세지를 사용할 것입니다.
...
h3(#conditional-validation). 상태에 따른 검증
...
+:if+와 +:unless+ 옵션과 검증이 실행되기 전에 실행될 이름의 심볼(Symbol)을 연결할 수 있습니다. 이것은 옵션의 가장 흔한 사용 방법입니다.
...
+eval+로 실행되는 유요한 루비코드의 문자열도 이용할 수 있습니다. 문자열이 정말 짧은 조건을 표현할때 이 옵션을 이용하세요.
...
여러분의 검증 헬퍼를 만들고 몇가지 다른 모델에 이를 재사용할 수 있습니다. 예를들어, 설문조사를 관리하는 어플리케이션은 선택 결과의 집합과 관계있는 필드를 표현하게 만드는게 유용하다는 점을 알 수 있습니다.
...
필드를 비운채 폼을 실행하면, 기본적으로 스타일을 비록 잃어버릴 지라도, 여러분은 흔히 뒤로가기를 합니다.:
...
예제를 위한 발판 작업(Scaffolding)은 위에서 보이듯이 붉은색 계열의 스타일로 정의된 +public/stylesheets/scaffold.css+를 생성한다.
...
콜백은 객체의 생명 주기의 특정한 순간에 호출되는 메소드입니다. 콜백을 이용해서, 엑티브 레코드 객제의 데이터베이스로 부터 생성, 저장, 갱신, 삭제, 검증, 혹은 로드 각 경우에 실행할 코드를 작성하는게 가능합니다.
...
여기에 사용 가능한 엑티브 레코드 콜백 전체 리스트가 있습니다. 개별 연상 호출되는 순서로 나열되어 있습니다.
...
+after_initialize+는 +new+를 사용하든 데이터베이스에서 정보를 로드하든, 엑티브 레코드 객체가 생성될 때마다 호출 됩니다. 그것은 직접 엑티브 레코드의 +initialize+ 메소드를 덮어쓰고(override) 싶을때 덮어쓰지 않고 기능을 추가할 수 있어서 유용합니다. 

+after_find+ 콜백은 데이터베이스에서 엑티브 레코드가 로드될 때마다 호출됩니다. +after_find+는 +after_initialize+ 전에 (양쪽 모두 정의되어 있다면,) 호출됩니다.

+after_initialize+ 와 +after_find+ 콜백은 다른 콜백과 약간 다릅니다. 그들에게는 반대 메소드인 +before_*+가 존재하지 않고, 표준 메소드처럼 정의하는 것이 그들을 등록할 수 있는 유일한 방법입니다. 만약 +after_initialize+ 나 +after_find+를 매크로 형태로 등록하려 시도하면, 이들은 무시됩니다. 이러한 동작은 성능면의 이유에 기인합니다. 왜냐하면, +after_initialize+와 +after_find+ 메소드가 데이터베이스에서 각 레코드를 호출할때마다 불리고, 눈에 띄게 쿼리를 느리게 만들수 있습니다.
