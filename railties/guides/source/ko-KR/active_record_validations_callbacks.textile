...
여기에 사용 가능한 엑티브 레코드 콜백 전체 리스트가 있습니다. 저마다의 작업 중에 호출되는 순서로 나열되어 있습니다.:
...
WARNING. 임의 예외 발생은 아마도 실패한 +save+ 류의 코드들을 중지 시킵니다. +ActiveRecord::Rollback+ 예외는 엑티브 레코드에게 롤백을 하라고 전달합니다. 이 예외는 내부적으로 잡히고, 상위로 전파되지 않습니다.
...
때로 여러분이 작성할 콜백 메소드는 다른 모델에 재사용 할만 하기도 합니다. 엑티브 레코드에는 콜백 메소드를 캡슐화한 클래스를 만드는 것도 가능합니다. 그래서 그들을 재사용하는건 매우 쉽습니다. 
