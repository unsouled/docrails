...
이 가이드는 레일스 라우팅에서 사용자에게 노출되는 기능들을 다룹니다.  이 가이드를 통해서, 아래와 같은 것들을 할 수 있습니다:
This guide covers the user-facing features of Rails routing. By referring to this guide, you will be able to:

* +routes.rb+ 코드의 이해
* Understand the code in +routes.rb+
* 권장되는 resoureful 스타일이나 <tt>match</tt> 메소드를 이용해, 자신만의 라우트를 구성
* Construct your own routes, using either the preferred resourceful style or the <tt>match</tt> method
* 액션이 파라메터를 넘겨 받는 방법의 이해
* Identify what parameters to expect an action to receive
* 라우트 헬퍼를 이용해 자동으로 패스와 URL 만들기
* Automatically create paths and URLs using route helpers
* 제약(constraints), Rack endpoiints 등과 같은 고급 기술의 사용
...
레일스 라우터는 URL 을 인식하고 컨트롤러의 액션으로 송출(dispatch) 합니다.  또 뷰에서 하드코딩된 문자열을 피하기 위해, path 와 URL 을 생성합니다.
The Rails router recognizes URLs and dispatches them to a controller's action. It can also generate paths and URLs, avoiding the need to hardcode strings in your views.

h4. URL 과 코드 연결하기

레일스 어플리케이션이 아래와 같은 요청을 받았을 때,
...
위 요청은 라우터에게 어떤 컨트롤러 액션이 매치되는지 물어봅니다.  만약 제일 먼저 매치되는 라우트가 아래와 같다면
...
해당 요청은 +patients+ 컨트롤러의 +show+ 액션에 <tt>{ :id => "17" }</tt> 값을 가진 +params+ 라는 변수와 함께 송출됩니다.
the request is dispatched to the +patients+ controller's +show+ action with <tt>{ :id => "17" }</tt> in +params+.

h4. 코드에서 패스와 URL 생성하기

당신은 path 와 url 을 생성할 수 있습니다.  만약 어플리케이션이 아래와 같은 코드를 포함한다면:
...
라우터는 +/patients/17+ 이라는 패스를 생성합니다.  이 패스는 뷰의 brittleness를 줄여주고 뷰의 코드를 더 읽기 편하게 만들어 줍니다.  라우터 헬퍼에서는 id 가 반드시 명시될 필요가 없다는 것을 기억해 주십시오.
...
리소스 라우팅은 resourceful 컨트롤러에 대한 일반적인 라우팅의 모든 것을 손쉽게 선언할 수 있게 해 줍니다.  +index+, +show+, +new+, +edit+, +create+, +update+, +destroy+ 액션을 각각의 라우팅으로 선언하는 것 대신, 한즐의 코드로 선언할 수 있게합니다.
