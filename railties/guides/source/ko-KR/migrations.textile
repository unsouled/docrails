...
h4(#being-talkative). 결과 출력하기(-수다스럽게 만들기)
...
성능 때문에 컬럼에 관한 정보는 캐시됩니다. 예를들어, 여러분이 테이블에 컬럼을 추가하고 나서 연관된 모델을 이용해서 새로운 데이터를 입력하려고 하면 모델은 과거 컬럼 정보를 이용하려 합니다. +reset_column_information+ 메소드를 이용하면 엑티브 레코드에게 강제로 컬럼 정보를 다시 읽으라고 명령할 수 있습니다. 예를 들면 다음과 같습니다.:
...
h4(#what-are-schema-files-for). 스키마 파일은 무엇을 위해 조냊하는가?

마이그레이션, 이들은 아마도 데이터베이스를 위한 믿을만한 소스가 되지는 못할 겁니다. 이건 +db/schema.rb+ 나 엑티브 레코드가 데이터베이스 조사해서 생성한 SQL 파일이 역할이죠. 이건 수정되도록 설계되어 있지 않고, 단지 현재 데이터베이스의 상태를 표현합니다. 

어플리케이션의 새로운 인스턴스를 배포할때 마이그레이션의 전체 과정을 재현할 필요는 없습니다. (그리고 이건 에러가 발생하기도 쉽죠) 현재 스키마의 기술된 내용을 데이터베이스에 적재하는 편이 훨씬 단순하고 빠릅니다.

예를 들어, 이건 테스트 데이터베이스를 만드는 방법입니다.:현재 개발 데이터베이스를 덤프하고(+db/schema.rb+ 든 +db/development.sql+ 이든) 나서 테스트 데이터베이스에 이것을 적재합니다. 
...
만은 방법 중 이것이 가장 정확합니다.  데이터베이스를 조사하고 +create_table+, +add_index+ 등을 이용하여 구조화된 표현으로 이 파일이 만들어 집니다. 이 파일은 데이터베이스 독립적이기 때문에 엑티브 레코드를 지원하는 어떤 데이터베이스에도 적재할 수 있습니다. 만약 여러분이 어플리케이션을 배포할때 다양한 데이터베이스를 대상으로 한다면 이는 매우 유용합니다. 
...
엑티브 레코드의 스키마 덤프 엔진 대신에, 해당 데이터베이스에 특화된 도구(+db:structure:dump+ Rake 태스크를 통해서)를 사용해서 +db{Rails.env}structure.sql+에 데이터베이스의 구조를 덤프 할 수 있습니다. 예를들어, PostgresSQL을 위해서는 +pg_dump+ 유틸리티를 사용하고, MySQL을 위해서는 다양한 테이블에 대한 +SHOW CREATE TABLE+을 결과를 파일에 저장 하면 됩니다. 스키마 적재는 내부에 포함된 SQL 구문 실행으로 간단하게 수행할 수 있습니다.

당연히, 이는 데이터베이스의 구조를 완벽하게 복제할 수 있는 방법이지만, 이 방법은 데이터베이스 이외에 스키마 정보를 로딩 하는걸 막게 될 것입니다.
...
엑티브 레코드의 방법은 데이터베이스보다 여러분의 모델을 더 영리하게 만들기를 주장합니다. 가령, 트리거나 외래키 제약 사항 같은 요소 같이 영리함(intelligence)을 데이터베이스로 넣는 요소의 과도한 사용을 권장하지 않습니다.

+validates :foreign_key, :uniqueness => true+과 같은 유효성 검사(validation)은 모델에서 데이터의 무결성을 지킬수 있는 방법입니다. 관계(association)에서 +:dependent+ 옵션은 부모 모델이 삭제되면 자동으로 자식 모델도 삭제합니다. 어플리케이션 레벨의 이러한 동작만으로 참조 무결성을 보장할 수는 없습니다. 그래서 몇몇 분들은 외래키 제약 사항을 강화합니다.

엑티브 레코드는 그러한 요소에 대해서 직접적인 어떤 도구도 지원하지는 않지만, +execute+ 메소드를 원하는 SQL 실행에 사용할 수 있습니다. 이와 관련해서 플러그인들도 다양합니다. 가령 "foreign_key_migrations":http://github.com/harukizaemon/redhillonrails/tree/master/foreign_key_migrations/ 플러그인은 외래키 추가 기능을 엑티브 레코드에 제공합니다. (+db/schema.rb+에 외래키 정보를 덤프하는 것도 포함합니다.)
...
* 2011년 2월 13일 초벌 번역 완료 by "Sangmin Ryu":http://neocoin.net
